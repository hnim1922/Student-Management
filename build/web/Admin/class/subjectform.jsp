<%-- 
    ...
    Created on : Feb 28, 2021, 9:09:25 PM
    Author     : ADMIN
--%>
...
<html>
    ...
    <body>
        ...
        <c:set var="tList" value="${requestScope.tList}"/>
        <c:set var="headerMsg" value="${requestScope.msg}"/>
        <c:set var="action" value="${requestScope.action}"/>
        
        <h2> ${headerMsg} </h2>
        
        <font color="red">${requestScope.errorExisted}</font>
        ...
        <table width="600px" border="0px solid">
            <form action="ClassServlet" method="POST" name="f1">
                ...
                <tr> 
                    <td>Semester </td> 
                    <td>: <input type="text" name="semester" value="" /><font color="red">${requestScope.errorSemester}</td> 
                </tr>
                <tr> 
                    <td>Teacher Name </td> 
                    <td>: <select name="teacher" value="" >
                        <c:forEach items="${tList}" var="t">
                        <option value="${t.id}">${t.fn}</option>
                        </c:forEach>
                        </select>
                    </td>
                </tr>
