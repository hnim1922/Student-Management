<%-- 
    Document   : classForm
    Created on : Feb 13, 2021, 11:07:35 PM
    Author     : ADMIN
--%>

<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="DTO.Score"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Subject form</title>
        <style type="text/css">
            body{
                   background: url("data:image/jpeg;base64,");
                   background-repeat:no-repeat; 
                   background-size: cover;
            }
        </style>
    </head>
    <body>
        <c:set var="sc" value="${requestScope.scObject}"/>
        <c:set var="sList" value="${requestScope.sList}"/>
        <c:set var="headerMsg" value="${requestScope.msg}"/>
        <c:set var="action" value="${requestScope.action}"/>
        
        <h2> ${headerMsg} </h2>

        <table width="600px" border="0px solid">
            <form action="ScoreServlet" method="POST" name="f1">
                <input type="hidden" name ="action" value="${action}">
                <tr> 
                    <td>Student ID </td> 
                    <c:if test="${sc.student.id!=''}">
                        <td>: <input type="text" name="student_id" value="${sc.student.id}" readonly/></td> 
                    </c:if>
                    <c:if test="${sc.student.id==''}">
                    <td>: <input type="text" name="student_id" value=""/><font color="red">${requestScope.errorID}</font></td> 
                    </c:if>
                </tr>
                <tr> 
                    <td>Subject ID </td> 
                    <c:if test="${sc.subject.id!=''}">
                        <td>: <input type="text" name="subject_id" value="${sc.subject.id}" readonly/></td> 
                    </c:if>
                    <c:if test="${sc.subject.id==''}">
                    <td>: <select name="subject_id" value="" >
                        <c:forEach items="${sList}" var="s">
                        <option value="${s.id}">${s.name}</option>
                        </c:forEach>
                        </select>
                    </td>
                    </c:if>
                </tr>
                <tr> 
                    <td>Score </td> 
                    <td>: <input type="text" name="score" value="${sc.score}" /><font color="red">${requestScope.errorScore}</font></td> 
                </tr>
                <tr> 
                    <td>Semester </td> 
                    <td>: <input type="text" name="semester" value="${sc.semester}" /><font color="red">${requestScope.errorSemester}</font></td> 
                </tr>
                <tr> 
                    <td>Status </td> 
                    <td>: <input type="text" name="status" value="${sc.status}" /><font color="red">${requestScope.errorStatus}</font></td> 
                </tr>
                <tr> 
                    <td colspan="2" align="center"> 
                        <input onclick="return confirm('Are you sure?')" type="submit" value="${action}" /> 
                        <input type="reset" value="Reset" />
                    </td> 
                </tr>
               
            </form>
        </table>
        <br> <br>
        <a href="ScoreServlet"> List of Scores </a>
    </body>
</html>
