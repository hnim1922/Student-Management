<%-- 
    Document   : studentform
    Created on : Feb 11, 2021, 3:30:03 PM
    Author     : ADMIN
--%>

<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="DTO.User"%>
<%@page import="DTO.Student"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
       <style type="text/css">
            body{
                   background: url("data:image/jpeg;base64,");
                   background-repeat:no-repeat; 
                   background-size: cover;
            }
        </style>
    </head>  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Student form</title>
        <style type="text/css">
            body{
                   background: url("data:image/jpeg;base64,");
                   background-repeat:no-repeat; 
                   background-size: cover;
            }
        </style>
    </head>
    <body>
        <c:set var="s" value="${requestScope.sObject}"/>
        <c:set var="u" value="${requestScope.uObject}"/>
        <c:set var="headerMsg" value="${requestScope.msg}"/>
        <c:set var="action" value="${requestScope.action}"/>
        
        <h2> ${headerMsg} </h2>

        <table width="600px" border="0px solid">
            <form action="StudentServlet" method="POST" name="f1">
                <input type="hidden" name ="action" value="${action}">
                <tr> 
                    <td>Student ID </td> 
                    <c:if test="${s.id == ''}">
                    <td>: <input type="text" name="id" value=""/><font color="red">${requestScope.errorID}</font></td>
                    </c:if>
                    <c:if test="${s.id != ''}">
                    <td>: <input name="id" value="${s.id}" readonly></td>
                    </c:if>
                </tr>
                <tr> 
                    <td>Last Name </td> 
                    <td>: <input type="text" name="lastName" value="${s.lastName}" /><font color="red">${requestScope.errorLn}</font></td> 
                </tr>
                <tr> 
                    <td>First Name </td> 
                    <td>: <input type="text" name="firstName" value="${s.firstName}" /><font color="red">${requestScope.errorFn}</font></td> 
                </tr>
                <tr> 
                    <td>Major Id </td> 
                    <td>: <input type="text" name="major" size="10" value="${s.majorId}" /><font color="red">${requestScope.errorMajor}</font></td> 
                </tr>
                <tr> 
                    <td>Semester</td> 
                    <td>: <input type="text" name="semester" size="15" value="${s.semester}" /><font color="red">${requestScope.errorSemester}</font></td> 
                </tr>
                <tr> 
                    <td>Class ID </td> 
                    <td>: <input type="text" name="classId" value="${s.classes.id}" /><font color="red">${requestScope.errorClass}</font></td> 
                </tr>
                <tr> 
                    <td>Email</td> 
                    <c:if test="${s.email == ''}">
                    <td>: <input type="text" name="email" value=""/> <font color="red">${requestScope.errorEmail}</font></td>
                    </c:if>
                    <c:if test="${s.email != ''}">
                    <td>: <input name="email" value="${s.email}" readonly></td>
                    </c:if>
                </tr>
                <tr> 
                    <td>Password</td> 
                    <td>: <input type="text" name="password" value="${u.password}" /><font color="red">${requestScope.errorPass}</font></td> 
                </tr>
                <tr> 
                    <td colspan="2" align="center"> 
                        <input onclick="return confirm('Are you sure?')" type="submit" value="${action}" /> 
                        <input type="reset" value="Reset" />
                    </td> 
                </tr>
               
            </form>
        </table>
        <br> <br>
        <a href="StudentServlet"> List of students </a>
    </body>
</html>
